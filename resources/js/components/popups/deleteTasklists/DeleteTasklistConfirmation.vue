  ...
  <div>
    <div class="mb-8">
      ...
      <ul class="list-disc mb-4">
        <li class="ml-4">{{ currentTasklist.name }}</li>
      </ul>
      <p>
        This action cannot be undone. The following tasks will also be deleted:
      </p>
      <ul class="list-disc mb-4">
        <li
...
export default {
  name: "DeleteTasklistConfirmation",
  props: {
    componentData: {
      type: Object,
      required: true,
  ...
  methods: {
    async deleteTasklist() {
      ...
      const tasklistName = this.currentTasklist.name;
      const tasklistId = this.currentTasklist.tasklist_id;
      const res = await this.TasklistsStore.deleteCurrentTasklist(tasklistId);
      if (res.success) {
        this.isDeleting = false;
        this.TasklistsStore.clearTasklistFromState(tasklistId);
        this.redirectToAcknowledgement(tasklistName);
        return;
      }
      this.isDeleting = false;
    ...
    redirectToAcknowledgement(name) {
      ...
      this.PopupStore.setComponent("DeleteTasklistAcknowledgement");
    },
    redirectToError(message) {
      this.PopupStore.setComponentData({
        message,
      });
      this.PopupStore.setComponent("DeleteTasklistError");
    },
  },
};
</script>
