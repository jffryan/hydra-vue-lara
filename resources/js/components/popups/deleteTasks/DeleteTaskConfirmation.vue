  ...
  <div>
    <div class="mb-8">
      ...
      <ul class="list-disc mb-4">
        <li class="ml-4">{{ currentTask.name }}</li>
      </ul>
...
<script>
import { usePopupStore, useTasksStore } from "@/stores";
...
export default {
  ...
  methods: {
    async deleteTask() {
      ...
      const res = await this.TasksStore.deleteCurrentTask(taskId);
      if (res.success) {
        this.isDeleting = false;
        this.TasksStore.clearTaskFromState(taskId);
        this.redirectToAcknowledgement(taskName);
        return;
      }
      this.isDeleting = false;
    ...
    redirectToAcknowledgement(name) {
      ...
      this.PopupStore.setComponent("DeleteTaskAcknowledgement");
    },
    redirectToError(message) {
      this.PopupStore.setComponentData({
        message,
      });
      this.PopupStore.setComponent("DeleteTaskError");
    },
  },
};
</script>
